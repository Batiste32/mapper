...
def get_field_metadata(field_name: str, db: Session = Depends(get_db)):
    meta = db.query(FieldMetadata).filter(FieldMetadata.field_name == field_name, FieldMetadata.visible == 1).first()
    if not meta:
        return {"field_name": field_name, "label": None, "description": None, "visible": False}
    return {"field_name": meta.field_name, "label": meta.label, "description": meta.descript, "visible": meta.visible}